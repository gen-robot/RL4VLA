...
class OpenVLAPolicy:
    def __init__(self, all_args, device_id: int):
        ...
        # )
        # self.action_head = self.action_head.to(torch.bfloat16).to("cuda:" + str(self.device_id))
        # self.action_head.eval()
        # # TODO: model_is_on_hf_hub， refer to openvla_oft/experiments/robot/openvla_utils.py:496
        ...
        #             filename="action_head--300000_checkpoint.pt",
        #         )
        #     )
        # )
    ...
    def _preprocess_obs(self, x: dict, action: torch.Tensor = None) -> BatchFeature:
        ...
        if action is None:
            ...
            task_prompt = [f"In: What action should the robot take to {t.lower()}?\nOut: {a}</s>" for t, a in zip(task_description, action_str)]
    ...
    def get_action(self, x: dict, deterministic) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        values, action, logprobs, generated_ids = self.vla.predict_action_batch(
            **features,
        ...
        assert len(action.shape) == 3 and action.shape[0] == values.shape[0]
        assert action.shape == generated_ids.shape
        assert len(logprobs.shape) == 2 and logprobs.shape[1] == 1

        return values, action, logprobs, generated_ids

    def get_action_temp(self, x: dict, do_sample, temperature, num_beams) -> tuple[torch.Tensor, torch.Tensor]:
        features = self._preprocess_obs(x)

        _, action, logprobs, generated_ids = self.vla.predict_action_batch(
            **features,
            unnorm_key=self.args.vla_unnorm_key,
            do_sample=do_sample,
            temperature=temperature,
...
class OpenVLAPPO:
    ...
    def train_ppo_step(self, idx, total, batch):
        obs_image, instruct, actions, generated_ids, value_preds, returns, masks, old_logprob, advantages = batch
        ...
        generated_ids = torch.tensor(generated_ids).to(self.tpdv["device"])  # int32
        generated_ids = generated_ids.reshape(generated_ids.shape[0], -1)  # [B, T, dim]
        ...
        # Policy loss
        logprob, entropy, values = self.policy.evaluate_actions(obs, generated_ids)
