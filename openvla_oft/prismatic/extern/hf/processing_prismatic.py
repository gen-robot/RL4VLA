...
class PrismaticImageProcessor(ImageProcessingMixin):
    ...
    def apply_transform(self, img: torch.Tensor) -> torch.Tensor:
        ...
        img: [B, C, H, W], assumed to be in [0, 255]
        """
        ...
        for idx in range(len(self.input_sizes)):
            ...
            img_idx = img_idx / 255.0  # Assuming input is in [0, 255]
            img_idx = TVF.normalize(img_idx, **self.tvf_normalize_params[idx])
            imgs_t.append(img_idx)

        # [Contract] `imgs_t` is a list of Tensors of shape [B, 3, H, W]; concatenate along dim=1
        img_t = torch.cat(imgs_t, dim=1)  # [B, 3 * n, H, W]
        return img_t
    ...
    def preprocess(
        ...
        **_: str,
        ...
        @param images: torch.Tensor [B, C, H, W], assumed to be in [0, 255]
