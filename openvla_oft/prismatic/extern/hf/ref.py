...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action_batch(
        ...
        """
        Predict actions for a batch of inputs, consistent with predict_action.

        Args:
            input_ids: Batch of input token ids (B, seq_len)
            attention_mask: Batch of attention masks (B, seq_len)
            pixel_values: Batch of pixel values (B, C, H, W)
            unnorm_key: Key for unnormalization statistics
            proprio: Batch of proprioceptive features (B, proprio_dim)
        ...
        Returns:
            ...
            - unnormalized_actions: (B, NUM_ACTIONS_CHUNK, ACTION_DIM)
            - action_hidden_states: (B, ACTION_DIM * NUM_ACTIONS_CHUNK, llm_dim)
        """
        batch_size = input_ids.shape[0]

        # Add special empty token if not present
        if not torch.all(input_ids[:, -1] == 29871):
            special_token = torch.full((batch_size, 1), 29871, dtype=input_ids.dtype, device=input_ids.device)
        ...
        # Prepare inputs by adding placeholder action tokens and stop token
        input_ids, attention_mask = self._prepare_input_for_action_prediction(input_ids, attention_mask)
        labels = self._prepare_labels_for_action_prediction(labels, input_ids)

        # Get input embeddings and action masks
        input_embeddings = self.get_input_embeddings()(input_ids)  # (B, seq_len, llm_dim)
        all_actions_mask = self._process_action_masks(labels)  # (B, seq_len)

        # Extract language embeddings
        language_embeddings = input_embeddings[~all_actions_mask].reshape(
            batch_size, -1, input_embeddings.shape[2]
        ...
        if use_diffusion:
            # Sample random noise for diffusion
            noise = torch.randn(
