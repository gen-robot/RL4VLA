...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _process_action_masks(self, labels):
        ...
        tricky_action_mask = torch.zeros_like(all_actions_mask, dtype=torch.bool)
        tricky_action_mask[:,-58:-2] = True  # Mask out the last 58 tokens (i.e. action tokens) for tricky actions
        return tricky_action_mask  # TODO TODO TODO
    ...
    def forward(
        ...
        if input_ids.shape[1] == 1:
            ...
            if attention_mask is not None:
                ...
                new_position_ids = multimodal_attention_mask.cumsum(dim=1) - 1
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _run_diffusion_prediction(
        ...
        curr_noisy_actions = curr_noisy_actions.reshape(curr_noisy_actions.shape[0], NUM_ACTIONS_CHUNK, ACTION_DIM)
    ...
    def _regression_or_discrete_prediction(
        ...
        if action_head is not None:
            ...
            normalized_actions = normalized_actions.reshape(normalized_actions.shape[0], NUM_ACTIONS_CHUNK, ACTION_DIM)
...
class OpenVLAForActionPredictionWithValueHead(PrismaticForConditionalGeneration):
    ...
    def __init__(self, config: OpenVLAConfig, vh_mode: str) -> None:
        ...
        if self.vh_mode == "a0" or self.vh_mode == "a55":
            self.value_head = ValueHead(config.text_config.hidden_size)
        elif self.vh_mode == "a":
            self.value_head = ValueHead(config.text_config.hidden_size * ACTION_DIM * NUM_ACTIONS_CHUNK)
        else:
            raise ValueError(f"Unknown value head mode: {self.vh_mode}")
        self.norm_stats = config.norm_stats
        # Compute action bins
        self.bins = torch.linspace(-1, 1, config.n_action_bins)
        self.bin_centers = (self.bins[:-1] + self.bins[1:]) / 2.0
        self.vocab_size = self.config.text_config.vocab_size - self.config.pad_to_multiple_of
    ...
    def evaluate_action(
        ...
        # check the token before action is ` `
        ...
        if self.vh_mode == "a0":
            ...
            raise ValueError(f"Unknown value head mode: {self.vh_mode}")
        logits_tensor = outputs.logits[:, -action_len - 2: -2, 32000 - 256 : 32000]
        logprobs_tensor = F.log_softmax(logits_tensor, dim=-1)
        idxes = (labels[:, -action_len - 1: -1] - (32000 - 256)).unsqueeze(-1)
        logprobs = torch.gather(logprobs_tensor, 2, idxes).squeeze(-1).sum(dim=1, keepdim=True)
        probs_tensor = F.softmax(logits_tensor, dim=-1)
        entropy = -(probs_tensor * logprobs_tensor).sum(dim=-1).mean(dim=-1, keepdim=True)
    ...
    def _regression_or_discrete_prediction(
        ...
        logits_tensor = language_model_output.logits[
            :,
            NUM_PATCHES + NUM_PROMPT_TOKENS : NUM_PATCHES + NUM_PROMPT_TOKENS + ACTION_DIM * NUM_ACTIONS_CHUNK,
        ] # [B, action_len, vocab_size + 64]
        logits_tensor = logits_tensor[:, :, 32000 - 256 : 32000] # [B, action_len, 256]
        predicted_action_token_ids = logits_tensor.argmax(dim=2) + (32000 - 256)  # [B, action_len]
        predicted_action_token_ids = predicted_action_token_ids.reshape(predicted_action_token_ids.shape[0], NUM_ACTIONS_CHUNK, ACTION_DIM)
        discretized_actions = self.vocab_size - predicted_action_token_ids
        if self.bin_centers.device != discretized_actions.device:
            self.bin_centers = self.bin_centers.to(discretized_actions.device)
        discretized_actions = torch.clip(discretized_actions - 1, min=0, max=self.bin_centers.shape[0] - 1)
        normalized_actions = self.bin_centers[discretized_actions]
        normalized_actions = normalized_actions.reshape(normalized_actions.shape[0], NUM_ACTIONS_CHUNK, ACTION_DIM)
        logprobs_tensor = F.log_softmax(logits_tensor, dim=-1)
        logprobs_tensor = torch.max(logprobs_tensor, dim=-1).values
        logprobs_tensor = logprobs_tensor.reshape(logprobs_tensor.shape[0], NUM_ACTIONS_CHUNK, ACTION_DIM)
        logprobs_tensor = logprobs_tensor.sum(dim=-1).sum(dim=-1, keepdim=True)  # [B, 1]

        return normalized_actions, last_hidden_states, logprobs_tensor, predicted_action_token_ids
    ...
    def get_value(
        ...
        hidden_features = outputs.hidden_states[-1][:, -1]
    ...
    def get_hidden(
        ...
        hidden_n1 = outputs.hidden_states[-1][:, -1]
        hidden_n2 = outputs.hidden_states[-2][:, -1]
        hidden_n3 = outputs.hidden_states[-3][:, -1]
        return torch.stack([hidden_n1, hidden_n2, hidden_n3], dim=1)
    ...
    def _unnormalize_actions(self, normalized_actions, unnorm_key=None):
        ...
        if ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.BOUNDS:
            mask = action_norm_stats.get("mask", torch.ones_like(torch.tensor(action_norm_stats["min"]), dtype=bool))
            action_high, action_low = torch.tensor(action_norm_stats["max"]), torch.tensor(action_norm_stats["min"])
        elif ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.BOUNDS_Q99:
            mask = action_norm_stats.get("mask", torch.ones_like(torch.tensor(action_norm_stats["q01"]), dtype=bool))
            action_high = torch.tensor(action_norm_stats["q99"], device=normalized_actions.device)
            action_low = torch.tensor(action_norm_stats["q01"], device=normalized_actions.device)
        else:
            raise ValueError("Unsupported action/proprio normalization type detected!")

        actions = torch.where(
            torch.tensor(mask, device=normalized_actions.device),
            0.5 * (normalized_actions + 1) * (action_high - action_low + 1e-8) + action_low,
            normalized_actions,
        )
    ...
    def predict_action_batch(
        ...
        **kwargs
        ...
        if use_proprio:
            NUM_PATCHES += 1
        normalized_actions, last_hidden_state, logprobs, generated_ids  = self._regression_or_discrete_prediction(
            input_embeddings,
            all_actions_mask,
            projected_patch_embeddings,
            attention_mask,
        ...
        if self.vh_mode == "a0":
            hidden_features = last_hidden_state[:, NUM_PATCHES + NUM_PROMPT_TOKENS]
            values = self.value_head(hidden_features)
        elif self.vh_mode == "a55":
            hidden_features = last_hidden_state[:, NUM_PATCHES + NUM_PROMPT_TOKENS + ACTION_DIM * NUM_ACTIONS_CHUNK]
            values = self.value_head(hidden_features)
        elif self.vh_mode == "a":
            hidden_features = last_hidden_state[:, NUM_PATCHES + NUM_PROMPT_TOKENS : NUM_PATCHES + NUM_PROMPT_TOKENS + ACTION_DIM * NUM_ACTIONS_CHUNK]
            hidden_features = hidden_features.view(hidden_features.shape[0], -1)  # [B, 7 * hidden_dim]
            values = self.value_head(last_hidden_state)
        else:
            raise ValueError(f"Unknown value head mode: {self.vh_mode}")
        return values, actions, logprobs, generated_ids
    ...
    def get_action_dim(self, unnorm_key: Optional[str] = None) -> int:
        # unnorm_key = self._check_unnorm_key(self.norm_stats, unnorm_key)
        # assert ACTION_DIM * NUM_ACTIONS_CHUNK == len(self.norm_stats[unnorm_key]["action"]["min"])
